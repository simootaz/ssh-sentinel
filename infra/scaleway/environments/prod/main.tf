# prod environment: one function, one database, three secrets.
# dev is the same wiring with its own state, project and sizing.

locals {
  # environments/prod -> environments -> scaleway -> infra -> repository root
